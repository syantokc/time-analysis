function [A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12, R_count] = read_restaurant_data_weekly (period_length, DBInfo)
    ...
    for restaurant = restaurants
        ...
        query_base = strcat('select period, sum(rating) as rating_sum, sum(review_count) as review_count from yelp_res.san_review_period_weekly where restaurantID = ''',char(restaurantID), '''');
        query_base_N = strcat('select period, sum(6*review_count-rating) as rating_sum, sum(review_count) as review_count from yelp_res.san_review_period_weekly where restaurantID = ''',char(restaurantID), '''');
