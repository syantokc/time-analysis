...
function [restaurants, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12, R_count] = read_data (period_length, DBInfo)
    conn = database(DBInfo.dbname, DBInfo.username, DBInfo.password, DBInfo.driver, DBInfo.dburl);
    query_i = 'select restaurantID from yelp_res.san_review_period group by restaurantID having max(period) >= ';
    query_i = strcat(query_i,num2str(period_length-1));
    query = strcat('select restaurantID from yelp_res.san_review_period where period <= ',num2str(period_length-1),' and restaurantID in (');
