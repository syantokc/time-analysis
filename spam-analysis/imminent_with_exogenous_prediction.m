function imminent_with_exogenous_prediction(cluster_number, period_length)
    ...
    %for i = working_set
        ...
        XX = get_var_features_normal(restaurantID,period,'Y');

        Y2 = YY(lag(k)+1:number_of_interval*interval+initial+lag(k),:);
        X2 = XX(lag(k)+1:number_of_interval*interval+initial+lag(k),:);
        ...
        Y1=cumsum(Y2);
        X1=cumsum(X2);
        [a,c] = var_for_single_restaurant_imminent_exogenous(i, initial, Y1, X1, number_of_interval, interval, k, lag_number, restaurant_name);
